...
uses Windows, SysUtils, Messages, Classes, Graphics, Controls,
  ...
  IBQuery, ADODB, Math, QExport, QExportRTF, frxpngimage, jpeg;

type
  TRFacturaFUNDGRUBEL = class(TQuickRep)
    QRBand1: TQRBand;
    lbFactura: TQRLabel;
    QRLabel4: TQRLabel;
    QRLabel5: TQRLabel;
...
var
  RFacturaFUNDGRUBEL: TRFacturaFUNDGRUBEL;
...
procedure TRFacturaFUNDGRUBEL.QRBand3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  a : integer;
begin
...
procedure TRFacturaFUNDGRUBEL.QDetalleCalcFields(DataSet: TDataSet);
var
...
procedure TRFacturaFUNDGRUBEL.ChildBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  t : string;
begin
...
procedure TRFacturaFUNDGRUBEL.QFacturaCalcFields(DataSet: TDataSet);
begin
  ...
  if not QFacturaNCF_Fijo.isnull then BEGIN
    QFacturaNumeroCF.Value := QFacturaNCF_Fijo.Text+formatfloat('00000000',QFacturaNCF_Secuencia.Value);
      //buscar vencimiento
...
procedure TRFacturaFUNDGRUBEL.CorregirError;
var
  mov : string;
begin
  dm.Query1.Close;
...
procedure TRFacturaFUNDGRUBEL.QRBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  Linea := 1;
end;

procedure TRFacturaFUNDGRUBEL.QRBand3AfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  Linea := Linea + 1;
...
procedure TRFacturaFUNDGRUBEL.QRBand4BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRFacturaFUNDGRUBEL.ChildBand3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  dm.Query1.Close;
  dm.Query1.SQL.Clear;
...
procedure TRFacturaFUNDGRUBEL.QRBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRFacturaFUNDGRUBEL.QFacturaAfterOpen(DataSet: TDataSet);
begin
...
procedure TRFacturaFUNDGRUBEL.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
var
  a : integer;
  cambiar : TQRLabel;
begin
 if FileExists('.\Transporte.txt') then begin
  ...
  if RFacturaFUNDGRUBEL.QRPrinter.PageNumber = RFacturaFUNDGRUBEL.QRPrinter.PageCount then
  begin
  ...
  QRLDBTotal2.Caption := FormatCurr('#,0.00',QFacturaFAC_TOTAL.Value*QFacturafac_tasa.Value);
  end
  else
  begin
