...
begin
  ...
  BalanceUS := BalanceUS + (QDocsMOV_MONTOUS.Value);
...
begin
 ...
 {   //Actualiza Balance NC
  ...
  Open;
  if not IsEmpty THEN
  QNCBalance.Value := QNCNCR_MONTO.Value-DM.Query1.FieldByName('USADO').Value;
  Close;
  end;
   }
  IF QNCBalance.Value = 0 THEN QRsdNC.Enabled := False ELSE
  BEGIN
 //Titin para imprimir monto en dolares.
   //qrlNCR_MONTO.Caption := format('%n',[QNCBalance.Value]);
   Balance := (Balance - QNCBalance.Value);
   BalanceUS := (BalanceUS - QNCNCR_MONTO_OMON.Value);

   QRLTasaNC.Caption := format('%n',[QNCncr_tasa.Value]);

   if QNCncr_tasa.Value > 1 THEN
   qrdbtxtNCR_MONTO.Enabled := False ELSE
   qrdbtxtNCR_MONTO.Enabled := True;
