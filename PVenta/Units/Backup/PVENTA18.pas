...
begin
 ...
 //   Puntero := QDetalle.GetBookmark;
    ...
    begin //--[2]--
      ...
      if (sPorcItbis > 0) and (ConItbis = 'True') then
      sPreciosinItbis := QDetalleDET_PRECIO.Value/sPorcItbis else
      sPreciosinItbis := QDetalleDET_PRECIO.Value ;
      end;

      if not (dm.QParametrospar_itbisincluido.Value = 'True') and (ConItbis = 'True') then begin
      sPreciosinItbis := QDetalleDET_PRECIO.Value ;
      end;
    ...
    if (err <> 0) then
      ...
      begin
        ...
        begin
          ...
          Msg1:=dm.PAD(' ','L',15,FormatFloat(',,,0.00',dm.Query1.FieldByName('monto').Value+RFactura.QFacturafac_devuelto.Value)) ;
          Msg2:=dm.PAD(' ','R',37,Msg2);
          err := DriverFiscal1.IF_WRITE('@PrintNonFiscalText|'+Msg2+Msg1);
        ...
        Devuelta:= FormatCurr('#,0.00',RFactura.QFacturafac_devuelto.Value);
 ...
 { if err <> 0 then
  ...
  begin
    ...
    begin
      ...
      err := DriverFiscal1.IF_WRITE('@TicketPayment|'+dm.Query1.FieldByName('fpa_codigo').AsString+'|'+
      floattostr(dm.Query1.FieldByName('monto').Value+RFactura.QFacturafac_devuelto.Value));

      dm.Query1.Next;
    end;

    IF RFactura.QFacturafac_devuelto.Value > 0 THEN
    err := DriverFiscal1.IF_WRITE('@TicketPayment|7|'+RFactura.QFacturafac_devuelto.AsString);
    ...
    //if (v_subtotal > (v_TotalPagado)) then
      ...
      begin
        ...
        if RFactura.QFacturaFAC_TOTAL.Value = v_TotalPagado then
        v_diferencia :=  0 else
        v_diferencia := RoundTo(RFactura.QFacturaFAC_TOTAL.Value - v_TotalPagado,2);
