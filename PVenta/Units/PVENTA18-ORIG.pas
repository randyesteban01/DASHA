...
uses
  ...
  ADODB, QuerySearchDlgADO, ComCtrls, Menus, IBCustomDataSet, dxmdaset,
  cxStyles, cxCustomData, cxGraphics, cxFilter, cxData, cxDataStorage,
  cxEdit, cxDBData, cxDropDownEdit, cxGridCustomTableView, cxGridTableView,
  cxGridDBTableView, cxGridLevel, cxClasses, cxControls, cxGridCustomView,
  cxGrid, cxDBLookupComboBox, frxClass, frxDBSet, cxContainer, cxTextEdit,
  cxMaskEdit, cxCalendar, QRPDFFilt, QRExport, QuickRpt, QRCtrls, vmaxFiscal,
  iFiscal, OCXFISLib_TLB, AppEvnts,Variants,Tfhkaif, frxBarcode, uJSON;
...
type
  ...
  public
    ...
    descuento, total, sSubTotal,sPreciosinItbis, TGralDesc, TGralItbis, sPorcItbis, TotalOtros, Limite, Balance, Comision, PorcFijo, MontoFijo : Currency;
    ...
    Debitos, Creditos : Currency;
    vEnvioEstadoCta : Boolean;
    cli_facturas, cli_records,idFacturaTmp, vl_suc, vl_ncr_numero : integer;
    vp_verifone:Boolean;
    ...
    procedure ImpTicketEnvio(vRecibe, vRecibeTel, vEnvia, vDescripcion, vCodigo, vCodigo2:String);
...
uses PVENTA37, PVENTA33, PVENTA34, PVENTA36, PVENTA40, PVENTA42,
  ...
  RVENTA128, PVENTA238, RVENTA41, RVENTA130;
...
var
  ...
  CtaCosto, CtaInvent, CtaIngreso, CtaDescuento, CtaPropinaLegal : String;
begin
...
if (RG_BuscaDet.ItemIndex = 2) and (Buscando = False) and (Not realizado) and (DesactivarCargaProductos = False) then begin
  ...
  begin
    dm.Query1.Close;
    dm.Query1.SQL.Clear;
    dm.Query1.SQL.Add('select UM.NOMBRE, UM.UnidadID from ProdUnidadMedida PUM');
    DM.Query1.SQL.Add('inner join UnidadMedida UM ON PUM.emp_codigo=UM.emp_codigo AND PUM.UNIDADID=UM.UNIDADID');
    dm.Query1.SQL.Add('WHERE UM.emp_codigo = :EMP AND PUM.pro_codigo = :COD');
    DM.Query1.SQL.Add('ORDER BY PUM.SECUENCIA');
    dm.Query1.Parameters.ParamByName('emp').Value := dm.QParametrosPAR_INVEMPRESA.Value;
...
//          dm.Query1.SQL.Add('select UM.NOMBRE, UM.UnidadID from ProdUnidadMedida PUM');
//          DM.Query1.SQL.Add('inner join UnidadMedida UM ON PUM.emp_codigo=UM.emp_codigo AND PUM.UNIDADID=UM.UNIDADID');
//          dm.Query1.SQL.Add('where UM.emp_codigo = :emp AND PUM.pro_codigo = :COD and PUM.UNIDADID = :UND');
//          DM.Query1.SQL.Add('ORDER BY PUM.SECUENCIA');
          dm.Query1.SQL.Add('select um.nombre from productos p');
          DM.Query1.SQL.Add('inner join unidadmedida um on p.emp_codigo=um.emp_codigo and p.unidadid=um.unidadid');
          dm.Query1.SQL.Add('where p.emp_codigo = :emp AND P.pro_codigo = :COD');
          dm.Query1.Parameters.ParamByName('emp').Value := dm.QParametrosPAR_INVEMPRESA.Value;
          dm.Query1.Parameters.ParamByName('COD').Value := QDetallePRO_CODIGO.Value;
          //dm.Query1.Parameters.ParamByName('UND').Value := QDetalleUnidadID.Value;
          dm.Query1.Open;
          ...
          begin
            ...
            begin
              ...
              QUnidades.Close;
              QUnidades.Parameters.ParamByName('emp').Value := QFacturaEMP_CODIGO.Value;
              ...
              IF QUnidades.RecordCount >  0 THEN BEGIN
        ...
        begin
          ...
          begin
            if dm.QParametrosPAR_FACMEDIDA.Value = 'False' then begin
              ...
              QDetalleDET_PRECIO.value := QDetalle.FieldByName('det_'+Precio).AsFloat else
              if dm.QParametrospar_itbisincluido.Value = 'True' then
              QDetalleDET_PRECIO.value := QDetalle.FieldByName('det_'+Precio).AsFloat/(1+(dm.Query1.fieldbyname('pro_montoitbis').asfloat/100)) else
              QDetalleDET_PRECIO.value := QDetalle.FieldByName('det_'+Precio).AsFloat;
              end;
          end;
         end;
...
if NOT dm.QParametrospar_busq_incrementada.Value = True then begin
  ...
  if ((RG_BuscaDet.ItemIndex <> 0) and (Grid.Columns.Items[0].Field.Value <> '') OR
     (RG_BuscaDet.ItemIndex = 0) and (Grid.Columns.Items[0].Field.Value <> 0)) then
  begin
//  if not (dm.QParametrosPAR_FACBAJARLINEA.Value = 'True') then begin
  Grid.SelectedIndex := Grid.SelectedIndex+1;
...
//if not Grid.Columns.Items[0].Field.IsNull then
//Grid.SelectedIndex := Grid.SelectedIndex+1;

if Grid.Columns.Items[0].Field.IsNull then
Grid.SelectedIndex := 0;
...
//        QDetalleDET_CANTIDAD.Clear;
      ...
      begin
        if (StrToFloat(format('%10.2F',[qVerProductos.FieldByName('costo').AsFloat])) > 0) and (dm.QParametrosPAR_DEBAJOCOSTO.Value = 'False') then
        begin
      ...
      begin
        ...
        begin
          if (dm.QParametrosPAR_DEBAJOPRECIO.Value <> 'True') and
...
begin
 ...
 //   Puntero := QDetalle.GetBookmark;
    ...
    begin //--[2]--
      ...
      if not (dm.QParametrospar_itbisincluido.Value = 'True') then begin
      sPreciosinItbis := QDetalleDET_PRECIO.Value ;
      end;
...
//Buscando Secuencia Factura_numero
    ...
    QFactura.UpdateBatch(arAll);
           ...
           ExecSQL;
              ...
              begin
              if not ((dm.QParametrospar_fac_preimpresa.Value = 'True') and (dm.QParametrospar_formato_preimpreso.Value <> 'QRAgregados')
                and (dm.QParametrospar_formato_preimpreso.Value <> 'Grabado_Exento')
                and (Impresora.IDPrinter > 0)) then
                begin
                ...
                if (dm.QParametrosPAR_PREGUNTAPEQ.Value = 'True') and
                 (MessageDLG('DESEA IMPRIMIR EN IMPRESORA PEQUEÑA?',mtConfirmation,
              ...
              //Imprimir Envio
              {TfrxBarCodeView(Rpt_Envio.FindObject('MCodigoBarra')).Text :=
              FormatFloat('0000000#',QEnviofac_numero.Value)+
              FormatFloat('000#',QEnvioIDEnvio.Value);
              TfrxMemoView(Rpt_Envio.FindObject('MRecibe')).Text :=
              'RECIBE: '+UpperCase(QEnvioNombre_Recibe.Text);
              TfrxMemoView(Rpt_Envio.FindObject('MRecibeTel')).Text :=
              'TEL RECIBE: '+UpperCase(QEnvioTelefono_Recibe.Text);
              TfrxMemoView(Rpt_Envio.FindObject('MEnvia')).Text :=
              'ENVIA: '+UpperCase(QEnvioNombre_envia.Text);
              TfrxMemoView(Rpt_Envio.FindObject('MDescripcion')).Text :=
              'DESC.: '+UpperCase(QEnvioDescripcion.Text);
              Rpt_Envio.PrepareReport();
              Rpt_Envio.Print;}
              ImpTicketEnvio(QEnvioNombre_Recibe.Text,QEnvioTelefono_Recibe.Text, QEnvioNombre_envia.Text,
               ...
               begin
                   if (dm.QParametrospar_formato_preimpreso.Value <> 'QRAgregados') then
                    ...
                    RFacturaSteelTec.Destroy;
                      ...
                      ELSE
                  {    end;
                if not (dm.QParametrospar_fac_preimpresa.Value = 'True') then begin
                     if (dm.QParametrospar_formato_preimpreso.Value <> 'QRAgregados') then
                    begin
                      application.CreateForm(tRFacturaPreImpresa, RFacturaPreImpresa);
                      RFacturaPreImpresa.QFactura.Parameters.ParamByName('emp').Value    := DM.vp_cia;
...
begin
 ...
 // DevolverExistencia;
  ...
  begin
    ...
    dm.Query1.close;
    dm.Query1.sql.clear;
    dm.Query1.sql.add('select fac_numero, fac_status, fac_abono, fac_fecha, fac_hold');
    dm.Query1.sql.add('from facturas');
    dm.Query1.sql.add('where emp_codigo = :emp');
    dm.Query1.sql.add('and tfa_codigo = :tipo');
    dm.Query1.sql.add('and fac_forma = :forma');
...
{        //datos del Envio
  ...
  writeln(arch, '              Descuento  :'+s1+format('%n',[RFactura.QFacturaTdesc_gral.Value]));
...
begin
  ...
  begin
    ...
    Query1.sql.add('d.det_precio1, d.det_precio2, d.det_precio3, d.det_precio4,');
...
function TfrmFactura.Producto_sin_Serializar(const key: string): boolean;
var a, fila:integer;
    Puntero : TBookmark;
begin
  Result := false;
  ...
  with QDetalle do
    begin
      ...
      begin
        ...
        QSerie.First;
         IF Trim(QSerieSER_NUMERO.Text) <> '' THEN
         a := a +1;
         QSerie.Next;
         end;
         end;
         Result := QDetalleDET_CANTIDAD.AsInteger <> a;
         if Result then
            begin
              QDetalle.EnableControls;
              exit;
...
begin
  if QDetallepro_unidad_medida.Value <> '' then begin
  if (not QDetallepro_unidad_medida.IsNull) and (dm.QParametrospar_inv_unidad_medida.Value = 'True')
  and (dm.QParametrosPAR_FACMEDIDA.Value <> 'True') then
  begin
    //buscando precio por unidad de medida
    QUnidades.Close;
     ...
     if RFactura.QDetalleDET_DESCUENTO.Value > 0 then
      begin
       // if ItbisIncluido = True then
         v_DescItems := ((RFactura.QDetalleDET_PRECIO.Value/(1+(QDetalleDET_ITBIS.Value/100)))*(RFactura.QDetalleDET_DESCUENTO.Value /100))*RFactura.QDetalleDET_CANTIDAD.Value;
        //v_DescItems:= RFactura.QDetalleTotalDescuento.Value * ((RFactura.QDetalleDET_ITBIS.Value /100)+1) ;
       err := DriverFiscal1.IF_WRITE('@TicketItem|'+copy(trim(RFactura.QDetallePRO_NOMBRE.value),1,v_TAMANO_MAX_DESCRIP)+'|'+RFactura.QDetalleDET_CANTIDAD.AsString+'|'+
        floattostr(RFactura.QDetalleTotalDescuento.Value + ((RFactura.QDetalleTotalDescuento.value * RFactura.QDetalleDET_ITBIS.Value)/100) )+'|'+FloatToStr(RFactura.QDetalleDET_ITBIS.Value)+'|D');
      end;
 ...
 { if err <> 0 then
  ...
  begin
    ...
    if (v_subtotal > (v_TotalPagado)) then
      ...
      err := DriverFiscal1.IF_WRITE('@TicketPayment|7|'+floattostr(RFactura.QFacturafac_devuelto.Value));
    ...
    if (err = 0) then
      begin
        ...
        if s_NIF <> EmptyStr then
          ...
          SQL.Clear;
          SQL.Add('UPDATE FACTURAS');
          SQL.Add('SET FAC_ITBIS = :FAC_ITBIS');
          SQL.Add('WHERE EMP_CODIGO = :EMP AND SUC_CODIGO = :SUC');
          SQL.Add('AND FAC_NUMERO = :FAC AND TFA_CODIGO = :TFA AND FAC_FORMA = :FOR');
          Parameters.ParamByName('FAC_ITBIS').Value := v_TotalItbis_From_Printer;
          Parameters.ParamByName('EMP').Value := QFacturaEMP_CODIGO.Value;
          Parameters.ParamByName('SUC').Value := QFacturaSUC_CODIGO.Value;
          Parameters.ParamByName('FAC').Value := QFacturaFAC_NUMERO.Value;
          Parameters.ParamByName('TFA').Value := QFacturaTFA_CODIGO.Value;
     ...
     case RFactura.QFacturatip_codigo.Value of
      //case TipoNCF of
        ...
        1:  err := DriverFiscal1.AbrirCF(copias,0, 0, '0001', IntToStr(RFactura.QFacturafac_caja.Value), RFactura.QFacturaNumeroCF.AsString, sNombreCliente, srncCliente, '');
            //con Valor Fiscal
        2:  err := DriverFiscal1.AbrirCF(copias,1, 0, '0001',IntToStr(RFactura.QFacturafac_caja.Value), RFactura.QFacturaNumeroCF.AsString, Trim(Copy(RFactura.QFacturaFAC_NOMBRE.AsString,1,40)), RFactura.QFacturafac_rnc.AsString, '');
            //Gubernamental
        3:  err := DriverFiscal1.AbrirCF(copias,1, 0, '0001',IntToStr(RFactura.QFacturafac_caja.Value), RFactura.QFacturaNumeroCF.AsString, Trim(Copy(RFactura.QFacturaFAC_NOMBRE.AsString,1,40)), RFactura.QFacturafac_rnc.AsString, '');
            //Regimen Especial
        4:  err := DriverFiscal1.AbrirCF(copias,101,0,'0001', IntToStr(RFactura.QFacturafac_caja.Value), RFactura.QFacturaNumeroCF.AsString, Trim(Copy(RFactura.QFacturaFAC_NOMBRE.AsString,1,40)), RFactura.QFacturafac_rnc.AsString, '');
      end; //fin case
...
procedure TfrmFactura.ImpTicketEnvio(vRecibe, vRecibeTel, vEnvia, vDescripcion, vCodigo, vCodigo2:String);
var
...
Parameters.ParamByName('suc').Value :=  QFacturaSUC_CODIGO.Value;
Parameters.ParamByName('fac').Value :=  QFacturaFAC_NUMERO.Value;
Parameters.ParamByName('tip').Value :=  QFacturaTFA_CODIGO.Value;
Parameters.ParamByName('for').Value :=  QFacturaFAC_FORMA.Value;
Open;
end;
...
procedure TfrmFactura.DBEdit8Exit(Sender: TObject);
begin
DBEdit19.SetFocus;
end;
