object frmBoleteria: TfrmBoleteria
  ...
  object QRESUMEN: TADOQuery
    ...
    SQL.Strings = (
        ...
        'DECLARE @EMP INT, @SUC INT, @FECHA DATETIME, @HORA CHAR(10), @US' +
        'UARIO INT'
      ''
      'SET @EMP = :EMP'
      'SET @SUC = :SUC'
      ...
      'SET @USUARIO = :USUARIO'
      ''
      '-- Actualizaci'#243'n de boletos'
      'UPDATE b'
      'SET suc_cod_destino = pb.suc_cod_destino'
      'FROM boleto b'
      'INNER JOIN boleto_precio pb '
      '  ON pb.emp_codigo = b.emp_codigo '
      '  AND pb.descripcion = b.descripcion'
      'WHERE b.emp_codigo = @EMP '
      '  AND b.suc_cod_origen = @SUC'
      '  AND b.fecha_valida = @FECHA '
      '  AND b.hora_valida = @HORA'
      '  AND B.usu_codigo = @USUARIO;'
      ''
      '-- Consulta principal usando CTE'
      'WITH CTE_Boletos AS ('
      '    SELECT s.loc_nombre AS SUCURSAL, '
      '           COUNT(b.boleto) AS CANT, '
      '           b.suc_cod_destino AS DESTINO'
      '    FROM boleto b'
      '    INNER JOIN localidades s '
      '      ON b.suc_cod_origen = s.suc_codigo '
      '      AND b.emp_codigo = s.emp_codigo '
      '      AND b.suc_cod_destino = s.loc_codigo'
      '    WHERE b.suc_cod_origen = @SUC '
      '      AND b.emp_codigo = @EMP'
      '      AND b.fecha_valida = @FECHA '
      '      AND b.hora_valida = @HORA'
      #9'  AND B.usu_codigo = @USUARIO'
      '    GROUP BY s.loc_nombre, b.suc_cod_destino'
      ')'
      'SELECT * '
      'FROM CTE_Boletos'
      'WHERE DESTINO IN ('
      '    SELECT suc_cod_destino '
      '    FROM boleto_precio '
      '    WHERE suc_cod_origen = @SUC'
      ');')
    Left = 800
    Top = 216
  ...
  object qBalance: TADOQuery
    ...
    SQL.Strings = (
      ...
      'AND A.HORA_VALIDA = @HORA AND u.usu_codigo = @USUARIO'
