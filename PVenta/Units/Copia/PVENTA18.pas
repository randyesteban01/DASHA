...
type
  TfrmFactura = class(TForm)
    ...
    procedure Introducirn1Click(Sender: TObject);
...
uses PVENTA37, PVENTA33, PVENTA34, PVENTA36, PVENTA40, PVENTA42,
  ...
  SIGMA06, RVENTA122, PVENTA15, PVENTA138, uEnvios;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                QUnidades.Close;
                QUnidades.Parameters.ParamByName('emp').Value := dm.vp_cia;//dm.QParametrosPAR_INVEMPRESA.Value;
                QUnidades.Parameters.ParamByName('pro').Value := QDetallePRO_CODIGO.Value;
                QUnidades.Parameters.ParamByName('und').Value := QDetalleUnidadID.Value; //QDetallepro_unidad_medida.Value;
                QUnidades.Open;
...
//        realizado := false;
      ...
      begin
        ...
        begin
          ...
          frmEnvio.ShowModal;
          if length(trim(frmEnvio.edoCiudad.Text)) > 0 then
          begin
...
procedure TfrmFactura.GridEnter(Sender: TObject);
begin
  Grid.selectedindex := 0;
end;
...
begin
  ...
  begin
    ...
    begin
      dm.Query1.Close;
      dm.Query1.SQL.Clear;
      dm.Query1.SQL.Add('select * from pr_verifica_disp (:emp, :alm, :pro, :can, :med, :usu, :suc, :tfa, :for, :num, :emp2)');
      dm.Query1.Parameters.ParamByName('emp').Value  := dm.vp_cia;//dm.QParametrosPAR_INVEMPRESA.Value;
...
//fin conduce
              ...
              begin
                  ...
                  begin
                    ...
                    begin
                      ...
                      begin
                        ...
                        RFactura.QDetalle.Parameters.ParamByName('par_invempresa').Value := dm.vp_cia;//dm.QParametrosPAR_INVEMPRESA.Value;
                    ...
                    begin
                      ...
                      begin
                        ...
                        RFacturaElegante.QDetalle.Parameters.ParamByName('par_invempresa').Value := dm.vp_cia;//dm.QParametrosPAR_INVEMPRESA.Value;
                ...
                begin
                  ...
                  RFacturaGrabadoExento.QDetalle.Parameters.ParamByName('par_invempresa').Value := dm.vp_cia;//dm.QParametrosPAR_INVEMPRESA.Value;
                ...
                begin
                  ...
                  begin
                    ...
                    RFactura2Columnas.QDetalle.Parameters.ParamByName('par_invempresa').Value := dm.vp_cia;//dm.QParametrosPAR_INVEMPRESA.Value;
...
if not dm.pr_verifica_disp(dm.QParametrosPAR_INVEMPRESA.AsString,
                       ...
                       QFactura.Parameters.parambyname('numero').Value,
                       dm.QEmpresasEMP_CODIGO.AsString)then
...
begin
  ...
  begin
    ...
    Query1.sql.add('t.ven_codigo, t.cot_direccion, t.cot_localidad, t.cot_rnc,');
    ...
    Query1.sql.add('d.pro_unidad_medida, d.UnidadID, d.Medida_Precio1, d.Medida_Precio2, d.Medida_Precio3,');
    Query1.sql.add('d.Medida_Precio4, d.det_cant_unidad_medida, p.pro_servicio');
...
procedure TfrmFactura.QFacTMPNewRecord(DataSet: TDataSet);
begin
  QDetalleTMP.close;
...
begin
  ...
  begin
    ...
    Query1.sql.add('d.con_direccion, d.con_localidad, d.ven_codigo,');
    ...
    begin
      ...
      dm.Query1.sql.add('select cli_referencia, cli_codigo, cli_nombre, cli_balance,');
      dm.Query1.sql.add('cli_limite, cli_precio, cli_descuento, cli_cuenta,');
      dm.Query1.sql.add('cli_direccion, cli_localidad, cli_telefono, cli_fax, cli_cedula, cli_rnc');
      dm.Query1.sql.add('from clientes');
      dm.Query1.sql.add('where emp_codigo = :emp');
      dm.Query1.sql.add('and cli_Status = '+#39+'ACT'+#39);
      if dm.QParametrosPAR_CODIGOCLIENTE.value = 'I' then
...
procedure TfrmFactura.BitBtn3Click(Sender: TObject);
var
  PorcReal : Double;
  mayor : string;
begin
if not dm.pr_verifica_disp(dm.QParametrosPAR_INVEMPRESA.AsString,
                       ...
                       QDetalle.Parameters.parambyname('numero').Value,
                       {QDetalleFAC_NUMERO.AsString,}
                       QDetalleEMP_CODIGO.AsString)then
MessageDlg('NO HAY SUFICIENTE DE ESTE PRODUCTO PARA'+#13+
                   'FACTURAR ESTA CANTIDAD',mtError,[mbok],0)
else
  if dm.QUsuariosusu_descuento.Value = 'True' then
...
procedure TfrmFactura.Introducirn1Click(Sender: TObject);
var
  a, fila : integer;
begin
  ...
  frmSerieFactura.Tag := 18;
  frmSerieFactura.GridSerie.RowCount := trunc(QDetalleDET_CANTIDAD.AsInteger);
  for a := 0 to frmSerieFactura.GridSerie.RowCount-1 do  begin
    frmSerieFactura.GridSerie.Cells[0,a] := '';
    frmSerieFactura.GridSerie.Cells[1,a] := QDetallePRO_CODIGO.AsString;
  end;
  ...
  //fernando 2017 05 22 limpiar grid Serie
  for a := 0 to frmSerieFactura.GridSerie.RowCount-1 do  begin
    frmSerieFactura.GridSerie.Rows[a].clear;
  end;
...
begin
  ...
  begin
    ...
    begin
      Close;
      Clear;
      Add('select p.Precio, p.Cantidad, p.Credito, u.UnidadID');  //p.Minimo,u.Nombre, 
      SQL.Add('from ProdUnidadMedida p, UnidadMedida u');
      Add('where p.emp_codigo = u.emp_codigo');
      Add('and p.UnidadID = u.UnidadID');
      Add('and p.emp_codigo = :emp');
      Add('and p.pro_codigo = :pro');
      Add('and u.Nombre     = :und');
      Parameters.ParamByName('emp').Value := dm.vp_cia;//dm.QParametrosPAR_INVEMPRESA.Value;
      Parameters.ParamByName('pro').Value := QDetallePRO_CODIGO.Value;
      Parameters.ParamByName('und').Value := TField(Sender).AsString;
      Open;
      if IsEmpty then
         begin
      ...
      begin
        if actbalance = 'False' then
           QDetalleDET_PRECIO.Value := fieldbyname('Precio').Value
        else
           QDetalleDET_PRECIO.Value := fieldbyname('Credito').Value;
        qdetalledet_cant_unidad_medida.Value := fieldbyname('Cantidad').Value;
        QDetalleUnidadID.Value := fieldbyname('UnidadID').Value;
      end;
    end;
  end;
end;

procedure TfrmFactura.QFacturaAfterEdit(DataSet: TDataSet);
begin
  accion := 2;
end;
  ...
  //  begin
    ...
    with frmSerieFactura do
      begin
        ...
        QSerie.Parameters.ParamByName('PRO_CODIGO').Value := QDetallePRO_CODIGO.Value;
        QSerie.Insert;
...
procedure TfrmFactura.QSerieNewRecord(DataSet: TDataSet);
begin
//  QSerie.Parameters.ParamByName('PRO_CODIGO').Value    := QDetallePRO_CODIGO.Value; 
...
{  DataSet.FieldByName('EMP_CODIGO').Value  := dm.vp_cia;
  DataSet.FieldByName('FAC_FORMA').Value   := QFacturaFAC_FORMA.value;

  DataSet.FieldByName('SUC_CODIGO').Value  := QFacturaSUC_CODIGO.Value;
  DataSet.FieldByName('TFA_CODIGO').Value  := StrToInt(trim(edTipo.Text));
}
end;

procedure TfrmFactura.QSerieAfterInsert(DataSet: TDataSet);
begin
