object frmSolicitud: TfrmSolicitud
  Left = 525
  Top = 292
  ActiveControl = DBEdit1
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'Solicitud de Cheques - [Crear / Modificar]'
  ClientHeight = 415
  ClientWidth = 691
  Color = clBtnFace
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  FormStyle = fsMDIChild
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  Visible = True
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnKeyPress = FormKeyPress
  OnPaint = FormPaint
  OnShow = FormShow
  DesignSize = (
    691
    415)
  PixelsPerInch = 96
  TextHeight = 13
  object Label8: TLabel
    Left = 520
    Top = 358
    Width = 47
    Height = 13
    Anchors = [akLeft, akBottom]
    Caption = 'Cr'#233'ditos'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label9: TLabel
    Left = 520
    Top = 375
    Width = 43
    Height = 13
    Anchors = [akLeft, akBottom]
    Caption = 'D'#233'bitos'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label10: TLabel
    Left = 520
    Top = 392
    Width = 44
    Height = 13
    Anchors = [akLeft, akBottom]
    Caption = 'Balance'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object btfindcuenta: TSpeedButton
    Left = 8
    Top = 387
    Width = 129
    Height = 25
    Anchors = [akLeft, akBottom]
    Caption = '[ F4 ] - Buscar Cuenta   '
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = btfindcuentaClick
  end
  object btdelete: TSpeedButton
    Left = 144
    Top = 387
    Width = 129
    Height = 25
    Anchors = [akLeft, akBottom]
    Caption = '[ F6 ] - Eliminar Cuenta'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    OnClick = btdeleteClick
  end
  object spcentros: TSpeedButton
    Left = 283
    Top = 389
    Width = 66
    Height = 22
    Caption = 'Centros'
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    OnClick = spcentrosClick
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 691
    Height = 200
    Align = alTop
    BevelInner = bvRaised
    BevelOuter = bvLowered
    TabOrder = 0
    object Label4: TLabel
      Left = 12
      Top = 32
      Width = 29
      Height = 13
      Caption = 'Banco'
    end
    object btBanco: TSpeedButton
      Left = 116
      Top = 32
      Width = 23
      Height = 22
      Flat = True
      Glyph.Data = {
        
        777770000000}
      OnClick = btBancoClick
    end
    object Label2: TLabel
      Left = 152
      Top = 56
      Width = 29
      Height = 13
      Caption = 'Fecha'
    end
    object Label3: TLabel
      Left = 462
      Top = 8
      Width = 30
      Height = 13
      Caption = 'Monto'
    end
    object Label5: TLabel
      Left = 462
      Top = 56
      Width = 51
      Height = 13
      Caption = 'Descuento'
    end
    object Label7: TLabel
      Left = 12
      Top = 152
      Width = 55
      Height = 13
      Caption = 'Beneficiario'
    end
    object Label1: TLabel
      Left = 12
      Top = 56
      Width = 37
      Height = 13
      Caption = 'N'#250'mero'
      FocusControl = DBEdit2
    end
    object Label14: TLabel
      Left = 12
      Top = 8
      Width = 40
      Height = 13
      Caption = 'Sucursal'
    end
    object SpeedButton7: TSpeedButton
      Left = 5
      Top = 80
      Width = 57
      Height = 22
      Caption = 'Concepto'
      Flat = True
      OnClick = SpeedButton7Click
    end
    object Label6: TLabel
      Left = 462
      Top = 80
      Width = 48
      Height = 13
      Caption = 'Retencion'
    end
    object Label11: TLabel
      Left = 462
      Top = 104
      Width = 24
      Height = 13
      Caption = 'Total'
    end
    object Label12: TLabel
      Left = 462
      Top = 32
      Width = 23
      Height = 13
      Caption = 'Tasa'
    end
    object lbLocSuc: TLabel
      Left = 14
      Top = 180
      Width = 44
      Height = 13
      Caption = 'Localidad'
    end
    object btLocalidad: TSpeedButton
      Left = 122
      Top = 178
      Width = 20
      Height = 19
      Flat = True
      Glyph.Data = {
        
        777770000000}
      OnClick = btLocalidadClick
    end
    object DBEdit1: TDBEdit
      Left = 72
      Top = 32
      Width = 41
      Height = 21
      BevelKind = bkFlat
      BorderStyle = bsNone
      CharCase = ecUpperCase
      DataField = 'BAN_CODIGO'
      DataSource = dsSolicitud
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
    end
    object tBanco: TEdit
      Left = 144
      Top = 32
      Width = 313
      Height = 21
      BevelKind = bkFlat
      BorderStyle = bsNone
      Color = clInfoBk
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
    end
    object DBEdit3: TDBEdit
      Left = 192
      Top = 56
      Width = 65
      Height = 21
      BevelKind = bkFlat
      BorderStyle = bsNone
      DataField = 'SOL_FECHA'
      DataSource = dsSolicitud
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 4
    end
    object DBEdit4: TDBEdit
      Left = 518
      Top = 8
      Width = 80
      Height = 21
      BevelKind = bkFlat
      BorderStyle = bsNone
      DataField = 'SOL_MONTO'
      DataSource = dsSolicitud
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 8
    end
    object DBEdit5: TDBEdit
      Left = 518
      Top = 56
      Width = 80
      Height = 21
      BevelKind = bkFlat
      BorderStyle = bsNone
      Color = clInfoBk
      DataField = 'SOL_DESCUENTO'
      DataSource = dsSolicitud
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 10
    end
    object DBEdit6: TDBEdit
      Left = 72
      Top = 80
      Width = 385
      Height = 21
      BevelKind = bkFlat
      BorderStyle = bsNone
      DataField = 'SOL_CONCEPTO1'
      DataSource = dsSolicitud
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 5
    end
    object DBEdit7: TDBEdit
      Left = 72
      Top = 104
      Width = 385
      Height = 21
      BevelKind = bkFlat
      BorderStyle = bsNone
      DataField = 'SOL_CONCEPTO2'
      DataSource = dsSolicitud
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 6
    end
    object DBEdit8: TDBEdit
      Left = 200
      Top = 152
      Width = 398
      Height = 21
      BevelKind = bkFlat
      BorderStyle = bsNone
      CharCase = ecUpperCase
      DataField = 'SOL_BENEF'
      DataSource = dsSolicitud
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 11
    end
    object btBenef: TBitBtn
      Left = 72
      Top = 152
      Width = 81
      Height = 21
      Caption = 'Buscar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 12
      TabStop = False
      OnClick = btBenefClick
      Glyph.Data = {
        
        777770000000}
    end
    object rbTipo: TDBRadioGroup
      Left = 599
      Top = 2
      Width = 91
      Height = 173
      DataField = 'SOL_TIPO'
      DataSource = dsSolicitud
      Items.Strings = (
        'Proveedores'
        'Clientes'
        'Empleados'
        'Gastos')
      TabOrder = 13
      Values.Strings = (
        'SUP'
        'CLI'
        'EMP'
        'GAS')
      OnClick = rbTipoClick
    end
    object DBEdit2: TDBEdit
      Left = 72
      Top = 56
      Width = 73
      Height = 21
      BevelKind = bkFlat
      BorderStyle = bsNone
      DataField = 'SOL_NUMERO'
      DataSource = dsSolicitud
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 3
    end
    object DBLookupComboBox2: TDBLookupComboBox
      Left = 72
      Top = 8
      Width = 385
      Height = 21
      DataField = 'SUC_CODIGO'
      DataSource = dsSolicitud
      DropDownRows = 20
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      KeyField = 'suc_codigo'
      ListField = 'suc_nombre'
      ListSource = dsSuc
      ParentFont = False
      TabOrder = 0
    end
    object DBEdit9: TDBEdit
      Left = 518
      Top = 80
      Width = 80
      Height = 21
      BevelKind = bkFlat
      BorderStyle = bsNone
      Color = clInfoBk
      DataField = 'SOL_RETENCION'
      DataSource = dsSolicitud
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 14
    end
    object DBEdit10: TDBEdit
      Left = 518
      Top = 104
      Width = 80
      Height = 21
      BevelKind = bkFlat
      BorderStyle = bsNone
      Color = clInfoBk
      DataField = 'SOL_MONTO_BANCO'
      DataSource = dsSolicitud
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 15
    end
    object DBEdit11: TDBEdit
      Left = 518
      Top = 32
      Width = 80
      Height = 21
      TabStop = False
      BevelKind = bkFlat
      BorderStyle = bsNone
      DataField = 'SOL_TASA'
      DataSource = dsSolicitud
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 9
    end
    object DBEdit12: TDBEdit
      Left = 72
      Top = 128
      Width = 385
      Height = 21
      BevelKind = bkFlat
      BorderStyle = bsNone
      DataField = 'SOL_CONCEPTO3'
      DataSource = dsSolicitud
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 7
    end
    object edCodigo: TEdit
      Left = 155
      Top = 152
      Width = 43
      Height = 21
      BevelKind = bkFlat
      BorderStyle = bsNone
      TabOrder = 16
      OnKeyDown = edCodigoKeyDown
    end
    object dbedtcont_numeroSucursal: TDBEdit
      Left = 72
      Top = 176
      Width = 46
      Height = 21
      BevelKind = bkFlat
      BorderStyle = bsNone
      CharCase = ecUpperCase
      DataField = 'cont_numeroSucursal'
      DataSource = dsSolicitud
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 17
      OnChange = dbedtcont_numeroSucursalChange
      OnKeyDown = dbedtcont_numeroSucursalKeyDown
    end
    object TSucLoc: TEdit
      Left = 146
      Top = 176
      Width = 311
      Height = 21
      BevelKind = bkFlat
      BorderStyle = bsNone
      Color = clInfoBk
      Enabled = False
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 18
    end
  end
  object lbCR: TStaticText
    Left = 578
    Top = 358
    Width = 103
    Height = 17
    Alignment = taRightJustify
    Anchors = [akLeft, akBottom]
    AutoSize = False
    BorderStyle = sbsSunken
    Caption = '0.00'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 2
  end
  object lbDB: TStaticText
    Left = 578
    Top = 375
    Width = 103
    Height = 17
    Alignment = taRightJustify
    Anchors = [akLeft, akBottom]
    AutoSize = False
    BorderStyle = sbsSunken
    Caption = '0.00'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 3
  end
  object lbBAL: TStaticText
    Left = 578
    Top = 392
    Width = 103
    Height = 17
    Alignment = taRightJustify
    Anchors = [akLeft, akBottom]
    AutoSize = False
    BorderStyle = sbsSunken
    Caption = '0.00'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 4
  end
  object btGrabar: TBitBtn
    Left = 8
    Top = 360
    Width = 129
    Height = 25
    Anchors = [akLeft, akBottom]
    Caption = '[ F2 ] - Grabar / Imprimir'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 5
    OnClick = btGrabarClick
  end
  object btLimpiar: TBitBtn
    Left = 144
    Top = 360
    Width = 129
    Height = 25
    Anchors = [akLeft, akBottom]
    Caption = '[ F3 ] - Limpiar Pantalla'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 6
    OnClick = btLimpiarClick
  end
  object btSalir: TBitBtn
    Left = 280
    Top = 360
    Width = 129
    Height = 25
    Anchors = [akLeft, akBottom]
    Caption = '[ F10 ] - Salir / Cancelar'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 7
    OnClick = btSalirClick
  end
  object PageControl1: TPageControl
    Left = 0
    Top = 200
    Width = 691
    Height = 159
    ActivePage = TabSheet1
    Align = alTop
    Anchors = [akLeft, akTop, akRight, akBottom]
    TabOrder = 1
    object TabSheet1: TTabSheet
      Caption = 'Cuentas contables'
      object GridCuentas: TDBGridObj
        Left = 0
        Top = 0
        Width = 683
        Height = 131
        Align = alClient
        DataSource = dsDetalle
        Options = [dgEditing, dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
        TabOrder = 0
        TitleFont.Charset = ANSI_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnColEnter = GridCuentasColEnter
        OnDrawColumnCell = GridCuentasDrawColumnCell
        OnEnter = GridCuentasEnter
        OnKeyDown = GridCuentasKeyDown
        RowHeight = 0
        Columns = <
          item
            Expanded = False
            Width = 65
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CAT_CUENTA'
            Title.Alignment = taCenter
            Title.Caption = 'Cuenta'
            Width = 104
            Visible = True
          end
          item
            Color = clInfoBk
            Expanded = False
            FieldName = 'CAT_NOMBRE'
            Title.Caption = 'Nombre de la Cuenta'
            Width = 337
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DET_ORIGEN'
            Title.Alignment = taCenter
            Title.Caption = 'Origen'
            Width = 53
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DET_MONTO'
            Title.Alignment = taCenter
            Title.Caption = 'Monto'
            Width = 95
            Visible = True
          end>
      end
    end
    object TabSheet2: TTabSheet
      Caption = 'Facturas de compra'
      ImageIndex = 1
      object lbFacturas: TStaticText
        Left = 0
        Top = 0
        Width = 467
        Height = 17
        Align = alTop
        Alignment = taCenter
        BorderStyle = sbsSunken
        Caption = 
          'FACTURAS O MOVIMIENTOS PARA PAGAR AL PROVEEDOR    (F8 - Aplicar ' +
          'saldo total)'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
      end
      object GriFacturas: TDBGrid
        Left = 0
        Top = 17
        Width = 683
        Height = 131
        Align = alClient
        Ctl3D = False
        DataSource = dsFacturas
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        Options = [dgEditing, dgTitles, dgColLines, dgRowLines, dgTabs, dgCancelOnExit]
        ParentCtl3D = False
        ParentFont = False
        TabOrder = 1
        TitleFont.Charset = ANSI_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnColEnter = GriFacturasColEnter
        OnEnter = GriFacturasEnter
        OnKeyDown = GriFacturasKeyDown
        Columns = <
          item
            Color = clInfoBk
            Expanded = False
            FieldName = 'FAC_NUMERO'
            ReadOnly = True
            Title.Alignment = taCenter
            Title.Caption = 'N'#250'mero'
            Width = 72
            Visible = True
          end
          item
            Alignment = taCenter
            Color = clInfoBk
            Expanded = False
            FieldName = 'FAC_FECHA'
            ReadOnly = True
            Title.Alignment = taCenter
            Title.Caption = 'Fecha'
            Width = 58
            Visible = True
          end
          item
            Alignment = taCenter
            Color = clInfoBk
            Expanded = False
            FieldName = 'FAC_VENCE'
            ReadOnly = True
            Title.Alignment = taCenter
            Title.Caption = 'Vence'
            Width = 59
            Visible = True
          end
          item
            Color = clInfoBk
            Expanded = False
            FieldName = 'FAC_TOTAL'
            ReadOnly = True
            Title.Alignment = taCenter
            Title.Caption = 'Total'
            Width = 74
            Visible = True
          end
          item
            Color = clInfoBk
            Expanded = False
            FieldName = 'FAC_ABONO'
            ReadOnly = True
            Title.Alignment = taCenter
            Title.Caption = 'Abono'
            Width = 69
            Visible = True
          end
          item
            Color = clInfoBk
            Expanded = False
            FieldName = 'FAC_ITBIS'
            Title.Alignment = taCenter
            Title.Caption = 'Itbis'
            Width = 62
            Visible = True
          end
          item
            Color = clInfoBk
            Expanded = False
            FieldName = 'FAC_SALDO'
            ReadOnly = True
            Title.Alignment = taCenter
            Title.Caption = 'Saldo'
            Width = 74
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'FAC_DESCUENTO'
            Title.Alignment = taCenter
            Title.Caption = 'Desc'
            Width = 52
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'FAC_RETENCION'
            Title.Alignment = taCenter
            Title.Caption = 'Retencion'
            Width = 58
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'FAC_PAGO'
            Title.Alignment = taCenter
            Title.Caption = 'Pago'
            Width = 74
            Visible = True
          end>
      end
    end
    object TabSheet4: TTabSheet
      Caption = 'Retenciones'
      ImageIndex = 3
      object Panel3: TPanel
        Left = 0
        Top = 0
        Width = 30
        Height = 148
        Align = alLeft
        BevelInner = bvRaised
        BevelOuter = bvLowered
        TabOrder = 0
        object SpeedButton5: TSpeedButton
          Left = 4
          Top = 4
          Width = 23
          Height = 22
          Hint = 'Eliminar producto de la lista'
          Flat = True
          Glyph.Data = {
            
            777770000000}
          ParentShowHint = False
          ShowHint = True
          OnClick = SpeedButton5Click
        end
        object SpeedButton6: TSpeedButton
          Left = 4
          Top = 26
          Width = 23
          Height = 22
          Hint = 'Nota para el producto'
          Flat = True
          Glyph.Data = {
            
            777770000000}
          ParentShowHint = False
          ShowHint = True
          OnClick = SpeedButton6Click
        end
      end
      object GridRet: TDBGrid
        Left = 30
        Top = 0
        Width = 653
        Height = 148
        Align = alClient
        Ctl3D = False
        DataSource = dsRet
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        Options = [dgEditing, dgTitles, dgColLines, dgRowLines, dgTabs, dgCancelOnExit]
        ParentCtl3D = False
        ParentFont = False
        TabOrder = 1
        TitleFont.Charset = ANSI_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnColEnter = GridRetColEnter
        OnEnter = GridRetEnter
        OnKeyDown = GridRetKeyDown
        Columns = <
          item
            Expanded = False
            FieldName = 'ret_codigo'
            Title.Alignment = taCenter
            Title.Caption = 'Codigo'
            Visible = True
          end
          item
            Color = clInfoBk
            Expanded = False
            FieldName = 'det_nombre'
            ReadOnly = True
            Title.Caption = 'Descripcion Retencion'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'det_monto'
            Title.Alignment = taCenter
            Title.Caption = 'Monto'
            Visible = True
          end>
      end
    end
  end
  object Search: TQrySearchDlgADO
    AliasName = DM.ADOSigma
    Left = 312
    Top = 216
  end
  object QSolicitud: TADOQuery
    Connection = DM.ADOSigma
    LockType = ltBatchOptimistic
    AfterInsert = QSolicitudAfterInsert
    AfterEdit = QSolicitudAfterEdit
    BeforePost = QSolicitudBeforePost
    AfterPost = QSolicitudAfterPost
    OnNewRecord = QSolicitudNewRecord
    DataSource = DM.dsEmpresas
    Parameters = <
      item
        Name = 'EMP_CODIGO'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'NUMERO'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'SUC'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'SELECT'
      'BAN_CODIGO, CLI_CODIGO, EMP_CODIGO, EMP_NUMERO, '
      'SOL_ANO, SOL_BENEF, SOL_CONCEPTO1, SOL_CONCEPTO2, '
      'SOL_DESCUENTO, SOL_FECANULO, SOL_FECHA, SOL_MES, '
      'SOL_MONTO, SOL_MOTIVOANULO, SOL_NUMERO, SOL_STATUS, '
      'SOL_TIPO, SUP_CODIGO, USU_CODIGO, SOL_CHEQUE,'
      'SOL_GENERACHEQUE, CEN_CODIGO, SOL_CONCEPTO3,'
      'SUC_CODIGO, SOL_RETENCION, SOL_MONTO_BANCO,'
      'SOL_TASA, cont_numeroSucursal'
      'FROM'
      'SOLICITUD'
      'WHERE'
      'EMP_CODIGO = :EMP_CODIGO'
      'AND SOL_NUMERO = :NUMERO'
      'AND SUC_CODIGO = :SUC')
    Left = 216
    Top = 216
    object QSolicitudBAN_CODIGO: TIntegerField
      FieldName = 'BAN_CODIGO'
      Origin = 'SOLICITUD.BAN_CODIGO'
      OnChange = QSolicitudBAN_CODIGOChange
    end
    object QSolicitudCLI_CODIGO: TIntegerField
      FieldName = 'CLI_CODIGO'
      Origin = 'SOLICITUD.CLI_CODIGO'
    end
    object QSolicitudEMP_CODIGO: TIntegerField
      FieldName = 'EMP_CODIGO'
      Origin = 'SOLICITUD.EMP_CODIGO'
    end
    object QSolicitudEMP_NUMERO: TIntegerField
      FieldName = 'EMP_NUMERO'
      Origin = 'SOLICITUD.EMP_NUMERO'
    end
    object QSolicitudSOL_ANO: TIntegerField
      FieldName = 'SOL_ANO'
      Origin = 'SOLICITUD.SOL_ANO'
    end
    object QSolicitudSOL_BENEF: TIBStringField
      FieldName = 'SOL_BENEF'
      Origin = 'SOLICITUD.SOL_BENEF'
      Size = 60
    end
    object QSolicitudSOL_CONCEPTO1: TIBStringField
      FieldName = 'SOL_CONCEPTO1'
      Origin = 'SOLICITUD.SOL_CONCEPTO1'
      Size = 80
    end
    object QSolicitudSOL_CONCEPTO2: TIBStringField
      FieldName = 'SOL_CONCEPTO2'
      Origin = 'SOLICITUD.SOL_CONCEPTO2'
      Size = 80
    end
    object QSolicitudSOL_DESCUENTO: TFloatField
      FieldName = 'SOL_DESCUENTO'
      Origin = 'SOLICITUD.SOL_DESCUENTO'
      OnChange = QSolicitudSOL_DESCUENTOChange
      currency = True
    end
    object QSolicitudSOL_FECANULO: TDateTimeField
      FieldName = 'SOL_FECANULO'
      Origin = 'SOLICITUD.SOL_FECANULO'
    end
    object QSolicitudSOL_FECHA: TDateTimeField
      FieldName = 'SOL_FECHA'
      Origin = 'SOLICITUD.SOL_FECHA'
      EditMask = 'cc-cc-cccc'
    end
    object QSolicitudSOL_MES: TIntegerField
      FieldName = 'SOL_MES'
      Origin = 'SOLICITUD.SOL_MES'
    end
    object QSolicitudSOL_MONTO: TFloatField
      FieldName = 'SOL_MONTO'
      Origin = 'SOLICITUD.SOL_MONTO'
      OnChange = QSolicitudSOL_MONTOChange
      currency = True
    end
    object QSolicitudSOL_MOTIVOANULO: TIBStringField
      FieldName = 'SOL_MOTIVOANULO'
      Origin = 'SOLICITUD.SOL_MOTIVOANULO'
      Size = 60
    end
    object QSolicitudSOL_NUMERO: TIntegerField
      FieldName = 'SOL_NUMERO'
      Origin = 'SOLICITUD.SOL_NUMERO'
      OnChange = QSolicitudSOL_NUMEROChange
    end
    object QSolicitudSOL_STATUS: TIBStringField
      FieldName = 'SOL_STATUS'
      Origin = 'SOLICITUD.SOL_STATUS'
      Size = 3
    end
    object QSolicitudSOL_TIPO: TIBStringField
      FieldName = 'SOL_TIPO'
      Origin = 'SOLICITUD.SOL_TIPO'
      Size = 3
    end
    object QSolicitudSUP_CODIGO: TIntegerField
      FieldName = 'SUP_CODIGO'
      Origin = 'SOLICITUD.SUP_CODIGO'
    end
    object QSolicitudUSU_CODIGO: TIntegerField
      FieldName = 'USU_CODIGO'
      Origin = 'SOLICITUD.USU_CODIGO'
    end
    object QSolicitudSOL_CHEQUE: TIntegerField
      FieldName = 'SOL_CHEQUE'
      Origin = 'SOLICITUD.SOL_CHEQUE'
    end
    object QSolicitudSOL_GENERACHEQUE: TIBStringField
      FieldName = 'SOL_GENERACHEQUE'
      Origin = 'SOLICITUD.SOL_GENERACHEQUE'
      Size = 1
    end
    object QSolicitudCEN_CODIGO: TIntegerField
      FieldName = 'CEN_CODIGO'
      Origin = 'SOLICITUD.CEN_CODIGO'
    end
    object QSolicitudSOL_CONCEPTO3: TStringField
      FieldName = 'SOL_CONCEPTO3'
      Size = 80
    end
    object QSolicitudSUC_CODIGO: TIntegerField
      FieldName = 'SUC_CODIGO'
      OnChange = QSolicitudSUC_CODIGOChange
    end
    object QSolicitudSOL_RETENCION: TBCDField
      FieldName = 'SOL_RETENCION'
      OnChange = QSolicitudSOL_RETENCIONChange
      currency = True
      Precision = 15
      Size = 2
    end
    object QSolicitudSOL_MONTO_BANCO: TBCDField
      FieldName = 'SOL_MONTO_BANCO'
      OnChange = QSolicitudSOL_MONTO_BANCOChange
      currency = True
      Precision = 15
      Size = 2
    end
    object QSolicitudSOL_TASA: TBCDField
      FieldName = 'SOL_TASA'
      currency = True
      Precision = 15
      Size = 2
    end
    object QSolicitudcont_numeroSucursal: TStringField
      FieldName = 'cont_numeroSucursal'
      Size = 5
    end
  end
  object dsSolicitud: TDataSource
    DataSet = QSolicitud
    Left = 248
    Top = 216
  end
  object QDetalle: TADOQuery
    Connection = DM.ADOSigma
    LockType = ltBatchOptimistic
    BeforePost = QDetalleBeforePost
    AfterPost = QDetalleAfterPost
    AfterDelete = QDetalleAfterDelete
    OnNewRecord = QDetalleNewRecord
    Parameters = <
      item
        Name = 'EMP'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'NUM'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'SUC'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'SELECT'
      'CAT_CUENTA, CAT_NOMBRE, DET_MONTO, DET_ORIGEN, '
      'DET_SECUENCIA, EMP_CODIGO, SOL_NUMERO, SUC_CODIGO'
      'FROM'
      'DET_SOLICITUD'
      'WHERE '
      'EMP_CODIGO = :EMP'
      'AND SOL_NUMERO = :NUM'
      'AND SUC_CODIGO = :SUC'
      'ORDER BY'
      'DET_SECUENCIA')
    Left = 216
    Top = 248
    object QDetalleCAT_CUENTA: TIBStringField
      FieldName = 'CAT_CUENTA'
      Origin = 'DET_SOLICITUD.CAT_CUENTA'
      OnChange = QDetalleCAT_CUENTAChange
      Size = 15
    end
    object QDetalleCAT_NOMBRE: TIBStringField
      FieldName = 'CAT_NOMBRE'
      Origin = 'DET_SOLICITUD.CAT_NOMBRE'
      Size = 60
    end
    object QDetalleDET_MONTO: TFloatField
      FieldName = 'DET_MONTO'
      Origin = 'DET_SOLICITUD.DET_MONTO'
      currency = True
    end
    object QDetalleDET_ORIGEN: TIBStringField
      FieldName = 'DET_ORIGEN'
      Origin = 'DET_SOLICITUD.DET_ORIGEN'
      Size = 7
    end
    object QDetalleDET_SECUENCIA: TIntegerField
      FieldName = 'DET_SECUENCIA'
      Origin = 'DET_SOLICITUD.DET_SECUENCIA'
    end
    object QDetalleEMP_CODIGO: TIntegerField
      FieldName = 'EMP_CODIGO'
      Origin = 'DET_SOLICITUD.EMP_CODIGO'
    end
    object QDetalleSOL_NUMERO: TIntegerField
      FieldName = 'SOL_NUMERO'
      Origin = 'DET_SOLICITUD.SOL_NUMERO'
    end
    object QDetalleSUC_CODIGO: TIntegerField
      FieldName = 'SUC_CODIGO'
    end
  end
  object dsDetalle: TDataSource
    DataSet = QDetalle
    Left = 248
    Top = 248
  end
  object QFacturas: TADOQuery
    Connection = DM.ADOSigma
    LockType = ltBatchOptimistic
    AfterInsert = QFacturasAfterInsert
    AfterPost = QFacturasAfterPost
    BeforeDelete = QFacturasBeforeDelete
    Parameters = <
      item
        Name = 'EMP'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'NUM'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'SUC'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'SELECT'
      'EMP_CODIGO, FAC_ABONO, FAC_DESCUENTO, FAC_FECHA, '
      'FAC_NUMERO, FAC_PAGO, FAC_SALDO, FAC_SECUENCIA, '
      'FAC_TOTAL, FAC_VENCE, SOL_NUMERO, SUP_CODIGO,'
      'SUC_CODIGO, FAC_ITBIS, FAC_RETENCION'
      'FROM'
      'DET_SOLFACTURAS'
      'WHERE'
      'EMP_CODIGO = :EMP'
      'AND SOL_NUMERO = :NUM'
      'AND SUC_CODIGO = :SUC'
      'ORDER BY'
      'FAC_FECHA')
    Left = 384
    Top = 216
    object QFacturasEMP_CODIGO: TIntegerField
      FieldName = 'EMP_CODIGO'
      Origin = 'DET_SOLFACTURAS.EMP_CODIGO'
    end
    object QFacturasFAC_ABONO: TFloatField
      FieldName = 'FAC_ABONO'
      Origin = 'DET_SOLFACTURAS.FAC_ABONO'
      currency = True
    end
    object QFacturasFAC_DESCUENTO: TFloatField
      FieldName = 'FAC_DESCUENTO'
      Origin = 'DET_SOLFACTURAS.FAC_DESCUENTO'
      OnChange = QFacturasFAC_DESCUENTOChange
    end
    object QFacturasFAC_FECHA: TDateTimeField
      FieldName = 'FAC_FECHA'
      Origin = 'DET_SOLFACTURAS.FAC_FECHA'
    end
    object QFacturasFAC_NUMERO: TIBStringField
      FieldName = 'FAC_NUMERO'
      Origin = 'DET_SOLFACTURAS.FAC_NUMERO'
      Size = 15
    end
    object QFacturasFAC_PAGO: TFloatField
      FieldName = 'FAC_PAGO'
      Origin = 'DET_SOLFACTURAS.FAC_PAGO'
      currency = True
    end
    object QFacturasFAC_SALDO: TFloatField
      FieldName = 'FAC_SALDO'
      Origin = 'DET_SOLFACTURAS.FAC_SALDO'
      currency = True
    end
    object QFacturasFAC_SECUENCIA: TIntegerField
      FieldName = 'FAC_SECUENCIA'
      Origin = 'DET_SOLFACTURAS.FAC_SECUENCIA'
    end
    object QFacturasFAC_TOTAL: TFloatField
      FieldName = 'FAC_TOTAL'
      Origin = 'DET_SOLFACTURAS.FAC_TOTAL'
      currency = True
    end
    object QFacturasFAC_VENCE: TDateTimeField
      FieldName = 'FAC_VENCE'
      Origin = 'DET_SOLFACTURAS.FAC_VENCE'
    end
    object QFacturasSOL_NUMERO: TIntegerField
      FieldName = 'SOL_NUMERO'
      Origin = 'DET_SOLFACTURAS.SOL_NUMERO'
    end
    object QFacturasSUP_CODIGO: TIntegerField
      FieldName = 'SUP_CODIGO'
      Origin = 'DET_SOLFACTURAS.SUP_CODIGO'
    end
    object QFacturasSUC_CODIGO: TIntegerField
      FieldName = 'SUC_CODIGO'
    end
    object QFacturasFAC_ITBIS: TBCDField
      FieldName = 'FAC_ITBIS'
      currency = True
      Precision = 15
      Size = 2
    end
    object QFacturasFAC_RETENCION: TBCDField
      FieldName = 'FAC_RETENCION'
      OnChange = QFacturasFAC_RETENCIONChange
      currency = True
      Precision = 15
      Size = 2
    end
  end
  object dsFacturas: TDataSource
    DataSet = QFacturas
    Left = 416
    Top = 216
  end
  object Query1: TADOQuery
    Connection = DM.ADOSigma
    Parameters = <>
    Left = 384
    Top = 248
  end
  object QCentro: TADOQuery
    Connection = DM.ADOSigma
    LockType = ltBatchOptimistic
    BeforePost = QCentroBeforePost
    OnNewRecord = QCentroNewRecord
    Parameters = <
      item
        Name = 'emp'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'num'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'suc'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'select'
      'emp_codigo,sol_numero,'
      'cen_codigo, det_secuencia, cen_nombre, det_monto,'
      'suc_codigo, cat_cuenta, cen_referencia, sub_referencia,'
      'sub_secuencia'
      'from'
      'SolicitudCentroCostos'
      'where'
      'emp_codigo = :emp'
      'and sol_numero = :num'
      'and suc_codigo = :suc'
      'order by'
      'det_Secuencia')
    Left = 384
    Top = 280
    object QCentrocen_codigo: TIntegerField
      FieldName = 'cen_codigo'
    end
    object QCentrodet_secuencia: TIntegerField
      FieldName = 'det_secuencia'
    end
    object QCentrocen_nombre: TStringField
      FieldName = 'cen_nombre'
      Size = 60
    end
    object QCentrodet_monto: TBCDField
      FieldName = 'det_monto'
      currency = True
      Precision = 15
      Size = 2
    end
    object QCentroemp_codigo: TIntegerField
      FieldName = 'emp_codigo'
    end
    object QCentrosol_numero: TIntegerField
      FieldName = 'sol_numero'
    end
    object QCentrosuc_codigo: TIntegerField
      FieldName = 'suc_codigo'
    end
    object QCentrocen_referencia: TStringField
      FieldName = 'cen_referencia'
      Size = 10
    end
    object QCentrosub_referencia: TStringField
      FieldName = 'sub_referencia'
      Size = 50
    end
    object QCentrosub_secuencia: TIntegerField
      FieldName = 'sub_secuencia'
    end
    object QCentrocat_cuenta: TStringField
      FieldName = 'cat_cuenta'
      Size = 15
    end
  end
  object dsCentro: TDataSource
    DataSet = QCentro
    Left = 416
    Top = 280
  end
  object QRetenciones: TADOQuery
    Connection = DM.ADOSigma
    LockType = ltBatchOptimistic
    BeforePost = QRetencionesBeforePost
    AfterPost = QRetencionesAfterPost
    AfterDelete = QRetencionesAfterDelete
    OnNewRecord = QRetencionesNewRecord
    Parameters = <
      item
        Name = 'emp'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'num'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'suc'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'select'
      'emp_codigo, sol_numero, ret_codigo, det_nombre,'
      'det_porciento, det_monto, det_secuencia, det_cuenta,'
      'suc_codigo'
      'from'
      'solicitudretencion'
      'where'
      'emp_codigo = :emp'
      'and sol_numero = :num'
      'and suc_codigo = :suc'
      'order by det_secuencia')
    Left = 384
    Top = 312
    object QRetencionesemp_codigo: TIntegerField
      FieldName = 'emp_codigo'
    end
    object QRetencionessol_numero: TIntegerField
      FieldName = 'sol_numero'
    end
    object QRetencionesret_codigo: TIntegerField
      FieldName = 'ret_codigo'
      OnChange = QRetencionesret_codigoChange
    end
    object QRetencionesdet_nombre: TStringField
      FieldName = 'det_nombre'
      Size = 60
    end
    object QRetencionesdet_porciento: TBCDField
      FieldName = 'det_porciento'
      Precision = 15
      Size = 2
    end
    object QRetencionesdet_monto: TBCDField
      FieldName = 'det_monto'
      OnChange = QRetencionesdet_montoChange
      currency = True
      Precision = 15
      Size = 2
    end
    object QRetencionesdet_secuencia: TIntegerField
      FieldName = 'det_secuencia'
    end
    object QRetencionesdet_cuenta: TStringField
      FieldName = 'det_cuenta'
      Size = 15
    end
    object QRetencionessuc_codigo: TIntegerField
      FieldName = 'suc_codigo'
    end
  end
  object dsRet: TDataSource
    DataSet = QRetenciones
    Left = 416
    Top = 312
  end
  object QSucursal: TADOQuery
    Connection = DM.ADOSigma
    LockType = ltReadOnly
    DataSource = DM.dsEmpresas
    Parameters = <
      item
        Name = 'emp_codigo'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end
      item
        Name = 'usu'
        Attributes = [paSigned]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'select'
      's.suc_codigo, s.suc_nombre, s.alm_codigo, s.emp_codigo'
      'from'
      'sucursales s, sucursal_acceso a'
      'where'
      's.emp_codigo = a.emp_codigo'
      'and s.suc_codigo = a.suc_codigo'
      'and a.emp_codigo = :emp_codigo'
      'and a.usu_codigo = :usu'
      'order by '
      's.suc_codigo')
    Left = 480
    Top = 280
    object QSucursalsuc_codigo: TIntegerField
      FieldName = 'suc_codigo'
    end
    object QSucursalsuc_nombre: TStringField
      FieldName = 'suc_nombre'
      Size = 60
    end
    object QSucursalalm_codigo: TIntegerField
      FieldName = 'alm_codigo'
    end
    object QSucursalemp_codigo: TIntegerField
      FieldName = 'emp_codigo'
    end
  end
  object dsSuc: TDataSource
    DataSet = QSucursal
    Left = 512
    Top = 280
  end
end
