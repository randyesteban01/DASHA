...
Updated by Angus Robertson, Magenta Systems Ltd, England, 3rd August 2011
...
GetProductInfo: function (dwOSMajorVersion, dwOSMinorVersion, dwSpMajorVersion,
      dwSpMinorVersion: DWORD; var dwReturnedProductType: DWORD): bool; stdcall;  // 10 Aug 2010
...
function IsWow64: boolean ;   // 14 Dec 2009 are we running under a 64-bit windows OS
var
              ...
              // 16 Aug 2010 Vista and later reports editions with new API and literals
                ...
                begin
                    if OsInfo.wProductType <= VER_NT_WORKSTATION then
                        ...
                        info := 'Windows 8' + inf2 ;  // 15 July 2011 - could be Windows 2012 
                end
                else
...
begin
    ...
    Temp := '';
    for I := 1 to L2 do
         Temp := AnsiString (ThousandSeparator) + Copy (S, LS - 3 * I + 1, 3) + Temp;  // 7 Aug 2010
    Result := Copy (S, N, (LS - 1) mod 3 + 1) + Temp;
    if N > 1 then Result := '-' + Result ;
end;
...
begin
    ...
    result := IntToStr (day) + result + space +
            LongMonthNames [month] + space + IntToStr (year) ;  
end ;
...
begin
    ...
    result := IntToStr (hours) + string (TimeSeparator) + info ;  // 7 Aug 2010
end ;
