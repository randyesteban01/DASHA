...
{
Magenta Systems WMI and SMART Component v5.3
Updated by Angus Robertson, Magenta Systems Ltd, England, 9th August 2010
...
begin
 ...
 // open device driver - 9x returns info on up to four drives
    ...
    begin
        hSMARTIOCTL := CreateFile(PChar ('\\.\SMARTVSD'), 0, 0, nil, CREATE_NEW, 0, 0) ;
     ...
     // get SMART version
        ...
        // The command can either be IDE identify or ATAPI identify.
                ...
                begin
                    with IdSector^ do
                    begin
                        model := ChangeByteOrders (@sModelNumber, Length (sModelNumber)) ;
                        // ChangeByteOrders (@sFirmwareRev, Length (sFirmwareRev))) ;
                        serial := ChangeByteOrders (@sSerialNumber, Length (sSerialNumber)) ;
...
begin
	...
	begin
		...
		TimeOutValue := 2 ;
		DataBufferOffset := PAnsiChar (@sptwb.bDataBuf) - PChar (@sptwb) ;
		SenseInfoOffset := PAnsiChar (@sptwb.bSenseBuf) - PChar (@sptwb) ;
    ...
    len := Ord((PAnsiChar(@sptwb.bDataBuf)+3)^) ;
	if ((PAnsiChar (@sptwb.bDataBuf) + 1)^ = #$80) and (len > 0) then
    begin
        SetString (S1, PAnsiChar (@sptwb.bDataBuf) + 4, len) ;
        serial := Trim (String (S1)) ;   // 9 Aug 2010
    end
    else
    begin
        result := false ;
 ...
 {   for len := 4 to 104 do
    ...
    SetString (S, PChar (@sptwb.bDataBuf) + 4, 100) ;
    model := Trim (S) ;}
    except
